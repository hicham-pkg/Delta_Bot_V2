    ...
    for (let name in global.plugins) {
      ...
      if (typeof plugin.all === "function") {
        try {
          ...
          for (let [jid] of global.owner.filter(
            ...
            if (data.exists)
              m.reply(
                `*[ ⚠️ Command report failed ⚠️ ]*\n\n*—◉ Plugin:* ${name}\n*—◉ User:* ${
                  m.sender
                }\n*—◉ Command:* ${m.text}\n\n*—◉ Error:*\n\`\`\`${format(
                  e
                )}\`\`\`\n\n*[❗] Report it to the creator of the bot to give him a solution. You can use the command #reporte*`.trim(),
                data.jid
              )
          }
        }
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (
          ...
          this.reply(
            m.chat,
            `*[❗𝐈𝐍𝐅𝐎 ❗] your diamonds have run out, you can buy more using the command ${usedPrefix}buy <cantidad>*`,
            m
          )
        ...
        if (plugin.level > _user.level) {
          this.reply(
            m.chat,
            `*[❗𝐈𝐍𝐅𝐎 ❗] level is required ${plugin.level} To use this command. your level is ${_user.level}*`,
            m
          )
        ...
        try {
          ...
          if (m.limit) m.reply(+m.limit + " Diamond 💎 Used")
        }
        break
      }
    }
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          try {
            ...
            this.sendFile(id, apii.data, "pp.jpg", text, null, false, {
              mentions: [user],
            })
          }
        }
    ...
    if (nk.isGroup == false) {
      if (nk.status == "offer") {
        let callmsg = await this.reply(
          nk.from,
          `Hi *@${nk.from.split("@")[0]}*, the ${
            nk.isVideo ? "video calls" : "calls"
          } are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unlock you!`,
          false,
          { mentions: [nk.from] }
        )
        ...
        await this.sendMessage(
          nk.from,
          {
            contacts: { displayName: "Alfa Stellar 👑", contacts: [{ vcard }] },
          },
          { quoted: callmsg }
        )
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] This command can only be used by the owner of the bot )*",
    owner:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] This command can only be used by the owner of the bot )*",
    mods: "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] This command can only be used by moderators and the owner of the bot*",
    premium:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] this command can only be used by premium users and the owner of the bot*",
    group: "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] this command can only be used in groups*",
    private:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] this command can only be used in the bots private chat*",
    admin:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] this command can only be used by group admins*",
    botAdmin:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] To be able to use this command it is necessary that bot be admin, turn on admin this number*",
    unreg:
      "*[ 🛑 Hey!! stop, you are not registered 🛑 ]*\n\n*—◉ to use this command you must register, use the command*\n*➣ #verificar nombre.edad*",
    restrict:
      "*[ ⚠️ Ａｌｅｒｔ ⚠️ ] This command is restricted/deactivated by decision of the owner of the bot*",
  }[type]
      ...
      extendedTextMessage: {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            thumbnail: image1,
            sourceUrl:
              "https://i.ibb.co/PG25cp2/Screenshot-20220304-214048.jpg",
          },
        },
      },
    },
